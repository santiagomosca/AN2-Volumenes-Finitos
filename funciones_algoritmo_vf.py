...
def matriz_jacobiano(tag_elem, datos):
    ...
    # Mi triangulo
    coord_nodo_local = elem_nodos(tag_elem, datos)
...
def coef_a_mn(tag_elem, datos):
    ...
    return A
#%
def matriz_global(M, tag_elem, datos):
    nodos = datos["elementos"][tag_elem]
    A = coef_a_mn(tag_elem, datos)
    m = 3 #nodos locales
    for i in range(3):
        n = nodos[i]
        M[tag_elem,n-1] = A[i]
...
def cc_dirichlet(dict_fisico, dict_nodos, dict_condiciones, matriz):
    ...
    for it in range(len(tag_grupos)):
        if tag_grupos[it] in nombres_grupo:
            ...
            print("")
            sys.exit(1)
    fuente = -np.dot(matriz, U_cond)
    matriz[:,U_cond.nonzero()] = 1          
