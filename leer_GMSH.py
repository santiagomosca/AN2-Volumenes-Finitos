...
def elem_from_msh(archivo):
    ''' Obtiene los tag de los nodos locales de los elementos. Cada fila corresponde a un elemento, 
    y cada columna a los nodos locales'''
    ...
    # Se determina en qué lineas comienza y termina el reporte de elementos del archivo:
    ...
    for i in range(nblocks):
        ...
        if dim==2:  # solo se toman nodos pertenecientes a superficies
            ...
            nmat += 1
            for j in range(nef):
                nodos_elem = [int(n) for n in malla[fila+1+j].split()[1:]]
                matrices.append(nodos_elem)
        fila += (1+nef)    
