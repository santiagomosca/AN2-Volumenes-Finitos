...
Matriz_Global = np.zeros((M,N))
print("Cantidad de elementos 'M': {:.0f}".format(M))
print("Cantidad de nodos 'N': {:.0f}".format(N))
    # Ciclo for (barrido de elementos)
for m in range(np.size(datos["elementos"],0)):
    G_m = funcion.elem_nodos(m, datos)
    A = funcion.coef_a_mn(m, datos)
    Matriz_Global = funcion.matriz_global(Matriz_Global, m, datos)  
